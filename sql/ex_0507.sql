...
-- 컬럼의 이름을 보기 좋게 변경
-- 테이블 이름을 짧게 줄인다.
-- 함수를 걸었을때 너무 길어지면 별명을 줄 수 있다.
-- select 컬럼명 as 별칭 from 테이블명
-- from 테이블명 as 별칭
-- as 생략 가능 

select category,avg_amount
from(select category,avg(sales_amount) as avg_amount
from sales
group by category
) as t; -- 서브쿼리로 조회된 테이블에는 별칭이 필수다.

-- 서브쿼리로 조회된 결과에는 이름이 없기 때문에 서브쿼리로 조회된 테이블에는 별칭이 필수다.
...
select region,max_price, min_price,max_price - min_price as price_gap
from(select region,MAX(sales_amount) as max_price,
min(sales_amount) as min_price from sales group by region
) d;

-- 카테고리별 총 판매금액을 조회 후 평균보다 카테고리 조회하기
-- inline view 를 사용해야 하는 이유
-- 1.카테고리별 총 판매 금액
-- 2. 그 결과들의 평균
...
-- right outer join
-- 오른쪽 테이블 기준으로 전부 출력하는 join
select name,class_name
from student s 
right outer join enroll e
on s.id = e.student_id ;

-- 모든 수강 기록을 기준으로 학생 이름을 붙여라
-- 실무에서는 left join을 더 자주 사용하고 right join은 방향만 바뀐 개념이라 잘 쓰지 않는 편이다

-- full outer join
-- 양쪽 테이블의 모든 데이터 조회
-- 일치하면 연결하고 일치하지 않으면 없는 쪽을 null로 표시한다.
-- mySQL은 full outer join을 직접 지원하지는 않는다.
-- left join 과 right join을 union으로 합쳐서 구현한다.

-- union : 두 가지 select 결과를 이어 붙인다.
-- 중복 제거를 자동으로 해준다.
...
-- 일반적인 조회에서는 많이 사용하지 않으나, 모든조합이 필요한 경우 사용
...
select c.customer_name,r.recommender_id
from customer c
left outer join customer r
on c.customer_id = r.recommender_id;
...
select concat(c.customer_name,'의 추천인은 ',r.customer_name)
from customer c
...
-- DB 전환 > roll up practice -> sakila

-- 고객 테이블과 주소테이블을 이용하여 고객의 이름,성,주소를 조회하라
...
-- 영화 번호,영화 제목,카테고리 ,대여료를 조회하는 select 문을 film_category_view 라는 이름의 view로 만들기

create view film_category_view as
select f.film_id,title,c.name,f.rental_rate
...
select c.last_name,c.first_name,count(r.rental_id) as rental_count
from customer c
inner join rental r
