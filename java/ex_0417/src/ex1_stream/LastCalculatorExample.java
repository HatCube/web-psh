...
public class LastCalculatorExample {
	public static void main(String[] args) {
		...
		// 특징
		// 1.스트림을 끝낸다.
		// >한번 사용이 끝난 스트림은 다시 사용할 수 없다.
		// 2.실제 실행을 일으킨다.
		// 중간 연산은 준비만 해두지만,최종연산은 실제 작업을 시작한다.
		// 3.결과를 만들거나 소비한다.
		...
		boolean result2 = numbers.stream().allMatch(x -> x % 2 == 0);
		System.out.println("결과 : " + result2);

		// none match()

		boolean result3 = numbers.stream().noneMatch(x -> x % 2 == 0);
		System.out.println("결과 : " + result3);
		
		//결과를 모으는 최종연산
		//collect()
		//스트림의 결과를 리스트,set,map 등으로 모으는 최종연산이다.
		...
		//Collectors.toSet() : 셋으로 변환
		//Collectors.toMap(K,V) : 맵으로 변환
		...
		//Optional 클래스
		//자바에서 값이 있을수도 있고 없을수도 있다는 상황을 안전하게 표현하기 위한 클래스
		
		//객체에 값이 들어있지 않으면 NPE(NullPointException)가 발생한다.
