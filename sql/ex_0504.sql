...
-- 카테고리 별 평균 결제 금액이 100,000 원 이상인 것만 조회
...
--   2025 - 01 - 02 이후의 거래 대상으로, 카테고리별 총액이 20,000 이상인 경우만 조회
...
-- 카테고리별 총 판매 금액을 구하되, 판매 금액이 200,000 이상인 카테고리만 조회
...
-- 카테고리별 가장 큰 판매금액을 구하되 가장 큰 금액이 200,000원 이상인 카테고리만 조회하기
...
-- 카테고리별 판매 총액을 구한 뒤, 총 판매 금액이 50,000 이상인 카테고리만 조회하기
select category, sum(amount)
from sales
group by category
having sum(amount) >= 50000;
-- 2025년 1월 1일에 판매된 데이터만 대상으로 하여 카테고리별 판매 총액 구하기 
-- 총 판매 금액이 20,000이상인 것만 조회
...
-- rollup
-- group by가 그룹별로 집계를 해준다면, rollup은 소계와 총계까지 한번에 만드는 기능
...
select count(commission_pct) from employees; -- 전체 행
-- 속성을 넣으면 null값을 제외함

-- sales 테이블에서 카테고리의 개수 세기
-- distinct : 중복되는값을 제거한다.
...
-- 사원 테이블에서 사원번호, 사원명, 급여를 조회하되, 급여는 급여 : XXX 형태로 나오도록 작성하라 
...
-- 사원 테이블에서 급여를 많이 받는 순으로 사번, 이름, 급여를 출력하라
